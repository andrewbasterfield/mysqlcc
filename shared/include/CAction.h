...
public:
  ...
  void setParentMenuText(const QString & s) { parentmenutext = s; }
  void setTmpAccel(const QKeySequence & k) { tmpaccel = k; }
  QKeySequence tmpAccel() const { return tmpaccel; }

private:
  void init();
  QString parentmenutext;
  QKeySequence tmpaccel;
};
